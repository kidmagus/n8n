...
export interface ScrapeEmployeeResult {
  CompanyName: string;
  "Company CVR": string;
  CXO: Array<Employee>;
  Bestyrelse: Array<Employee>;
  Director: Array<Employee>;
}

export async function scrapeEmployee(username: string, password: string, cvr: string): Promise<ScrapeEmployeeResult> {
  ...
  try {
    ...
    let visibleInput: Locator | null = null;
    ...
    const extractPeopleFromSection = async (sectionName: string) => {
