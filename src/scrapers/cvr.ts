...
export interface ScrapeCVRResult {
  CompanyName: string;
  "Company CVR": string;
}

export async function scrapeCVR(username: string, password: string, company: string): Promise<ScrapeCVRResult> {
  ...
  try {
    ...
    throw new Error(`CVR scraping failed: ${err instanceof Error ? err.message : "Unknown error"}`);
  } finally {
    await browser.close();
  }
}
 ...
 */
async function findVisibleLocator(page: any, selector: any) {
